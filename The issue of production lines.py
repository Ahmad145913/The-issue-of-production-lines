...
# Formats the given solution into a string with
...
# calculates how well the individual solution satisfies the constraints of and objectives of the problem.
def fitness(individual: dict[int, list[str]]):
    ...
    return round(total_dif / SHIFTS_COUNT, 2)
...
# performs a crossover operation between two parent solutions in order to create a child solution.
def crossover(parent1: dict[int, list[str]], parent2: dict[int, list[str]]):
    global tasks_count_divided_by_shifts_count
    # the new solution(child) created by the crossover
    ...
    for shift in range(SHIFTS_COUNT):
        child[shift] = []
        shift_tasks = []
        # Randomly select one of the parent solutions (random_parent) from parent1 and parent2.
        ...
        for task in random_parent[shift]:
            ...
            if task not in available_tasks:
                continue
            taskHasNoPrerequisites = task not in preqs.keys()
            # Check if the current task has no prerequisites (preqs).
            # If it does, add it to shift_tasks.
            if taskHasNoPrerequisites:
                shift_tasks.append(task)
            else:
                # If the current task has prerequisites, randomly determine the number of tasks
                #  to be assigned to the current shift.
...
# Mutates a given solution by randomly changing some of
# its tasks based on a predefined probability.
def mutation(solution):
    for shift in range(SHIFTS_COUNT):
        for task in range(len(solution[shift])):
            if random.random() < 0.1 and task in preqs.keys():
                # swap the task with one of its prerequisites
                solution[shift][task] = random.choice(preqs[solution[shift][task]])
    return solution


# Creates a new solution by performing crossover between two randomly selected
#  parent solutions and then applying mutation to the resulting child solution.
...
def executeAlgorithm():
    ...
    with tasks.open("r") as tasks_file, links.open("r") as links_file:
        ...
        # initialize as 300
        best_fitness = 300.0
        # Setup a directory to store output files
        ...
        with open(temp_dir + "/all_results_steps.txt", "w") as all_solutions_file, open(
            ...
            # clear output files if for some reason they contain an old data.
